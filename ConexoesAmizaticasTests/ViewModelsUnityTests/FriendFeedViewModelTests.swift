...
struct FriendFeedViewModelTests {
    ...
    func yOffsetIsOffscreenForFarItems() {
        let connection = makeConnection()
        // 10 posts garante que índice 5 fique distante de 0
        for _ in 0..<10 {
            connection.feedManager.addPost(Post(images: []))
        }
        ...
        let vm = FriendFeedViewModel(connection: connection)
        vm.draggingItem = 0
        // Item no índice 3 deveria estar fora de campo (distance ~3)
    ...
    func onDragChangedUpdatesDragging() {
        ...
        // DragGesture.Value não tem init público; uma alternativa é checar manualmente o cálculo.
        // Simulamos atribuindo diretamente o valor que o método produziria.
